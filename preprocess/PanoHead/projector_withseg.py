...
def run_projection(
    ...
    if target_img is not None:
        ...
        # 打开json文件，取出target——fname和c就行
        cam_path = os.path.join(target_img, 'dataset.json')
        with open(cam_path, 'r') as f:
            cam_data = json.load(f)

        fname_list = list(cam_data['labels'].keys())
        target_fname = fname_list[idx]
        fname = target_fname.split('.')[0]
        c_numpy = np.array(cam_data['labels'][target_fname]['label']).astype(np.float32)
        c = torch.from_numpy(c_numpy).unsqueeze(0).to(device)
        target_fname = os.path.join(target_img, target_fname)
