...
class TriPlaneGenerator(torch.nn.Module):
    ...
    def synthesis(self, ws, c, neural_rendering_resolution=None, update_emas=False, 
        ...
        feature_samples, depth_samples, weights_samples, normal_samples = self.renderer(planes, self.decoder, ray_origins, ray_directions, self.rendering_kwargs) # channels last
        ...
        normal_image = normal_samples.permute(0, 2, 1).reshape(N, normal_samples.shape[-1], H, W).contiguous()
        ...
        return {'image': sr_image, 'image_raw': rgb_image, 'image_depth': depth_image, "image_mask": mask_image, "image_normal": normal_image}
    ...
    def sample_rgb(self, coordinates, directions, ws, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        # Same as sample, but expects latent vectors 'ws' instead of Gaussian noise 'z'
        planes = self.backbone.synthesis(ws, update_emas = update_emas, **synthesis_kwargs)
        planes = planes.view(len(planes), 3, 32 * self.rendering_kwargs['triplane_depth'], planes.shape[-2], planes.shape[-1])
        return self.renderer.run_model_rgb(planes, self.decoder, coordinates, directions, self.rendering_kwargs)
