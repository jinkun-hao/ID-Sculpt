...
class MipRayMarcher2(nn.Module):
    ...
    def run_forward_norm(self, colors, densities, depths, rendering_options, normal=None):
        deltas = depths[:, :, 1:] - depths[:, :, :-1]
        colors_mid = (colors[:, :, :-1] + colors[:, :, 1:]) / 2
        densities_mid = (densities[:, :, :-1] + densities[:, :, 1:]) / 2
        depths_mid = (depths[:, :, :-1] + depths[:, :, 1:]) / 2
        normal_mid = (normal[:, :, :-1] + normal[:, :, 1:]) / 2
        ...
        composite_rgb = torch.sum(weights * colors_mid, -2)
        composite_normal = torch.sum(weights * normal_mid, -2)
        weight_total = weights.sum(2)
        composite_depth = torch.sum(weights * depths_mid, -2) / weight_total
    ...
    def forward(self, colors, densities, depths, rendering_options, normal=None):
        if normal is None:
            composite_rgb, composite_depth, weights = self.run_forward(colors, densities, depths, rendering_options)
        else:
            composite_rgb, composite_depth, weights, composite_normal = self.run_forward_norm(colors, densities,
                                                                       depths, rendering_options, normal=normal)
            return composite_rgb, composite_depth, weights, composite_normal
