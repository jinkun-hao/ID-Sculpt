...
class TDDFA(object):
    ...
    def recon_vers(self, param_lst, roi_box_lst, **kvs):
        ...
        rotate_flag = kvs.get('rotate_flag', False)
        size = self.size
        ...
        for param, roi_box in zip(param_lst, roi_box_lst):
            if dense_flag:
                ...
                if rotate_flag:
                    pts3d = (self.bfm.u + self.bfm.w_shp @ alpha_shp + self.bfm.w_exp @ alpha_exp). \
                        reshape(3, -1, order='F')
                else:
                ...
                R, offset, alpha_shp, alpha_exp = _parse_param(param)
                if rotate_flag:
                    pts3d = (self.bfm.u_base + self.bfm.w_shp_base @ alpha_shp + self.bfm.w_exp_base @ alpha_exp). \
                        reshape(3, -1, order='F')
                else:
