...
@threestudio.register("head3d-system")
class ImageConditionDreamFusion(BaseLift3DSystem):
    ...
    def training_substep(self, batch, batch_idx, guidance: str, render_type="rgb"):
        ...
        normal_temp = out['comp_normal']
        # rgb_temp = out['comp_rgb']
        
        # if guidance != "ref":
        #     normal_img = (normal_temp[..., [1, 2, 0]].detach().cpu().numpy() * 255)[0]
        #     # rgb_temp = (rgb_temp.detach().cpu().numpy() * 255)[0]
        #     #
        #     # rgb_ref_img = (gt_normal.detach().cpu().numpy() * 255)[0]
        #     control_img = (batch['ref_control_img'].permute(0, 2, 3, 1).detach().cpu().numpy() * 255)[0]
        #     concatenated_image = np.concatenate((normal_img, control_img), axis=1).astype('uint8')
        #     pil_image = Image.fromarray(concatenated_image)
            
        #     base_filename = "vis/comp_normal_viewspace.png"
        #     # 检查文件是否已存在
        #     if os.path.isfile(base_filename):
        #         # 文件已存在，查找可用的文件名
        #         i = 1
        #         while os.path.isfile(f'{base_filename}_{i:03d}.png'):
        #             i += 1
        ...
        if guidance == "ref":
            ...
            # guidance_out = self.guidance_ref(
            #     guidance_inp,
            #     batch['ref_control_img'],
            #     self.prompt_ref_utils,
            #     batch['elevation'],
            #     batch['azimuth'],
        ...
        if self.cfg.freq.ref_or_guidance == "accumulate":
            ...
            if hasattr(self.guidance.cfg, "only_pretrain_step"):
                if (self.guidance.cfg.only_pretrain_step > 0) and (
                    ...
                    do_guidance = True
                    do_ref = False
        elif self.cfg.freq.ref_or_guidance == "only_guidance":
            do_ref = False
            do_guidance = True
