...
@threestudio.register("sd-controlnet-vsd-ip1-guidance")
class ControlNetVSDGuidance(BaseObject):
    ...
    def configure(self) -> None:
        ...
        if self.cfg.control_type in ("normal", "input_normal"):
            ...
            controlnet_name_or_path = "../../../sd_model/models--CrucibleAI--ControlNetMediaPipeFace/snapshots/f6ed75cc495674bea8bf7409ef3d0e5bfb7d8c90/diffusion_sd15"
    ...
    def __call__(
        ...
        if self.cfg.control_type == "normal":
            image_cond = self.prepare_image_cond(normal_condition)
        else:
        ...
        # self.cfg.use_sds = False
        # if azimuth > -30 and azimuth < 30:
        #     self.cfg.use_sds = False
        ...
        if (
            ...
            # normal_img = load_image("vis/normal_test.png")

            # normal_cond = load_image("vis/normal_test.png")
            prompt = "the head portrait of a man with short hair, black shirt, detailed ear"
            # negative_prompt = "monochrome, lowres, bad anatomy, worst quality, low quality, blurry"

            pipe_kwargs = {"safety_checker": None, "feature_extractor": None, "requires_safety_checker": False,
                           "torch_dtype": self.weights_dtype, }
            controlnet = ControlNetModel.from_pretrained(
                "../../../sd_model/models--lllyasviel--control_v11p_sd15_normalbae/snapshots/cb7296e6587a219068e9d65864e38729cd862aa8",
                torch_dtype=torch.float16, )
            scheduler = DDIMScheduler.from_pretrained(self.cfg.ddim_scheduler_name_or_path, subfolder="scheduler",
                                                      torch_dtype=self.weights_dtype, )
            vae = AutoencoderKL.from_pretrained(self.cfg.vae_model_path).to(dtype=torch.float16)
            pipe = StableDiffusionControlNetPipeline.from_pretrained(self.cfg.pretrained_model_name_or_path,
                                                                     controlnet=controlnet, scheduler=scheduler,
                                                                     vae=vae, **pipe_kwargs).to(self.device)
            self.ip = IPAdapterFaceID(pipe, self.cfg.ip_ckpt, self.device, num_tokens=16)

            # from torchvision import transforms
            # # import torch.nn.functional as F
            # transform = transforms.ToTensor()
            # normal_img = transform(normal_img).unsqueeze(0).to(self.device).to(torch.float16)
            # normal_img = F.interpolate(normal_img, (512, 512), mode="bilinear", align_corners=False)

            # rgb_BCHW = normal_img

            # image_cond = image_cond[:, [2,0,1],:,:]
            # rgb_BCHW = rgb.permute(0, 3, 1, 2)
            def encode_images(imgs):
                input_dtype = imgs.dtype
                imgs = imgs * 2.0 - 1.0
                posterior = self.ip.pipe.vae.encode(imgs.to(torch.float16)).latent_dist
                latents = posterior.sample() * self.ip.pipe.vae.config.scaling_factor
                return latents.to(input_dtype)
            ...
            def get_latents(
                ...
                if rgb_as_latents:
                    ...
                    # encode image into latents with vae
                    latents = encode_images(rgb_BCHW_512)
                return latents

            latents, _ = self.get_latents(rgb_BCHW, rgb_as_latents=False)

            t = torch.randint(900, 901, [1], dtype=torch.long, device=self.device)
            # self.scheduler.config.num_train_timesteps = t.item()
            # self.scheduler.set_timesteps(30)
            # noise = torch.randn_like(latents)
            # latents2 = self.scheduler.add_noise(latents, noise, t)

            edit_latents = self.edit_latents(ip_text_embeddings, latents, rgb_BCHW, t, use_ip=True)
            edit_images = self.decode_latents(edit_latents)
            edit_images = F.interpolate(edit_images, (H, W), mode="bilinear")
            import torchvision.transforms as transforms
            from PIL import Image

            # 假设 edit_images 是一个形状为 [1, 3, 512, 512] 的 PyTorch 张量
            # edit_images = ...

            # 将张量从 GPU 移动到 CPU
            # edit_images = rgb_BCHW
            edit_images_cpu = edit_images.to("cpu")

            # 将张量的数据类型从 torch.float32 转换为 torch.uint8
            edit_images_uint8 = (edit_images_cpu * 255).clamp(0, 255).to(torch.uint8)

            # 使用 torchvision.transforms.ToPILImage() 将张量转换为 PIL 图像
            to_pil = transforms.ToPILImage()
            pil_image = to_pil(edit_images_uint8.squeeze(0))

            # 将 PIL 图像保存到文件
            pil_image.save("vis/normal_test_normal77.png")

            from diffusers.utils.torch_utils import randn_tensor

            # t = torch.randint(900,1000,[1],dtype=torch.long,device=self.device)
            scheduler.config.num_train_timesteps = t.item()
            scheduler.set_timesteps(self.cfg.diffusion_steps)
            noise = torch.randn_like(latents)
            latents1 = scheduler.add_noise(latents, noise, t)  # type: ignore

            images = self.ip.generate(
                prompt=prompt, negative_prompt='',
                faceid_embeds=faceid_embeds, image=rgb_BCHW, latents=latents1.to(torch.float16), num_samples=1,
                width=512, height=512, num_inference_steps=20, seed=2024)[0]

            # from PIL import Image
            # images.save('vis/normal_test_normal7.png')
