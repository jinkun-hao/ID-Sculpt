...
class PromptProcessorOutput:
    ...
    def get_text_embeddings(
        ...
        if_seperate: bool = False,
        if_null_text: bool = False,
        ...
        if view_dependent_prompting:
            ...
            null_text_embeddings = self.null_text_embeddings.expand(batch_size, -1, -1)
        ...
        if if_seperate:
            ...
            if if_null_text:
                return torch.cat([null_text_embeddings, uncond_text_embeddings], dim=0)
            else:
    ...
    def get_text_embeddings_perp_neg(
        ...
        text_embeddings = torch.cat(
            [
                ...
                torch.stack(uncond_text_embeddings, dim=0).squeeze(0),
...
class PromptProcessor(BaseObject):
    ...
    def configure(self) -> None:
        ...
        for d in self.directions:
            if d.name == "back":
                self.negative_prompts_vd.append(d.negative_prompt('face, nose, eyes, mouth, front face, facial'))
            elif d.name == "overhead":
                self.negative_prompts_vd.append(d.negative_prompt('face, nose, eyes, mouth, front face, facial, overexposure, Warm light'))
            else:
                self.negative_prompts_vd.append(d.negative_prompt(self.negative_prompt))
    ...
    def load_text_embeddings(self):
        ...
        self.null_text_embeddings = self.load_from_cache(self.null_prompt)[None, ...]
