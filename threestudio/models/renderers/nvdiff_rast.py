...
@threestudio.register("nvdiff-rast")
class NVDiffRasterizer(Rasterizer):
    ...
    def forward(
        ...
        **kwargs
        ...
        rast, _ = self.ctx.rasterize(v_pos_clip, mesh.f, (height, width))
        mask = rast[..., 3:] > 0
        mask_aa = self.ctx.antialias(mask.float(), rast, v_pos_clip, mesh.f)

        v_cam = torch.cat(
            [mesh.v, torch.ones([mesh.v.shape[0], 1]).to(mesh.v)], dim=-1
        )
        # actually disparity (1 / depth), to align with controlnet
        gb_depth, _ = self.ctx.interpolate_one(v_pos_clip[0,:, :3].contiguous(), rast, mesh.f)
        ...
        gb_depth_aa = self.ctx.antialias(
            gb_depth_aa, rast, v_pos_clip, mesh.f
        )


        out = {"opacity": mask_aa, "mesh": mesh, "depth": gb_depth_aa}
        ...
        if render_mask:
            ...
            with torch.no_grad():
                ...
                rast_front, _ = self.ctx.rasterize(v_pos_clip_front, mesh.f, (height, width))
                mask_front = rast_front[..., 3:]
                mask_front = mask_front[mask_front > 0] - 1.
                faces_vis = mesh.f[mask_front.long()]

                mesh._v_rgb = torch.zeros(mesh.v.shape[0], 1).to(mesh.v)
                mesh._v_rgb[faces_vis[:,0]] = 1.
                mesh._v_rgb[faces_vis[:,1]] = 1.
                mesh._v_rgb[faces_vis[:,2]] = 1.
                mask_vis, _ = self.ctx.interpolate_one(mesh._v_rgb, rast, mesh.f)
                mask_vis = mask_vis > 0.
                ...
                # v_pos_ = (torch.mm(torch.tensor(std2mesh).to(mesh.v), mesh.v.T).T) * 2
                ...
                # rgb_vis, _ = self.ctx.interpolate_one(mesh._v_rgb, rast, mesh.f)
                # rgb_vis_aa = self.ctx.antialias(
                #     rgb_vis, rast, v_pos_clip, mesh.f
                # )
                # from torchvision.utils import save_image
                # save_image(rgb_vis_aa.permute(0,3,1,2), "debug.png")
        

        gb_normal, _ = self.ctx.interpolate_one(mesh.v_nrm, rast, mesh.f)
        gb_normal = F.normalize(gb_normal, dim=-1)
        gb_normal_aa = torch.lerp(
            torch.zeros_like(gb_normal), (gb_normal + 1.0) / 2.0, mask.float()
        )
        gb_normal_aa = self.ctx.antialias(
            gb_normal_aa, rast, v_pos_clip, mesh.f
        )
        ...
        gb_normal_viewspace_aa = self.ctx.antialias(
            gb_normal_viewspace_aa, rast, v_pos_clip, mesh.f
        )
        ...
        if render_rgb:
            ...
            gb_pos, _ = self.ctx.interpolate_one(mesh.v, rast, mesh.f)  # [1, 512, 512, 3] 光栅化后每个像素点对应的三维位置坐标
            ...
            geo_out = self.geometry(positions, output_normal=False) # [142401, 3]
            ...
            gb_rgb_aa = self.ctx.antialias(gb_rgb, rast, v_pos_clip, mesh.f)
