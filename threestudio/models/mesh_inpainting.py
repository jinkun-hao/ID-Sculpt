...
class MeshInpainting:
    def __init__(
        ...
        self.device = v.device if device is None else device
        ...
        self.ori_center = 0
        self.ori_scale = 1
        print(f"[Mesh loading] v: {self.v.shape}, f: {self.f.shape}")
        if self.vn is None and not init_stage:
            self.auto_normal()
            print(f"[Mesh loading] vn: {self.vn.shape}, fn: {self.fn.shape}")
        if self.albedo is None and self.vt is None and not init_stage:
            ...
            print(f"[unwarp uv map......]")
            self.cache_path = f'.threestudio_cache/{self.v.shape[0]}'
            self.auto_uv(cache_path=self.cache_path)
            print(f"[Mesh loading] vt: {self.vt.shape}, ft: {self.ft.shape}")
        
        h = w = int(1024) # hard-code texture resolutioin
    ...
    def remove_outlier(self, outlier_n_faces_threshold: Union[int, float]):
        ...
        components = mesh_temp.split(only_watertight=False)
        ...
        mesh_temp = trimesh.util.concatenate(components)

        # convert back to our mesh format
        v_pos = torch.from_numpy(mesh_temp.vertices).to(self.v)
        t_pos_idx = torch.from_numpy(mesh_temp.faces).to(self.v)

        clean_mesh = MeshInpainting(v=v_pos, f=t_pos_idx)
    ...
    def write_glb(self, path):
        ...
        gltf = pygltflib.GLTF2(
            ...
            buffers=[
                pygltflib.Buffer(
                    byteLength=len(f_np_blob) + len(v_np_blob) + len(vn_np_blob) + len(vt_np_blob) + len(
                        albedo_blob))
            ],
