...
class Mesh:
    def __init__(
        self, v: Float[Tensor, "Nv 3"], f: Integer[Tensor, "Nf 3"], **kwargs
    ) -> None:
        self.v: Float[Tensor, "Nv 3"] = v
        self.f: Integer[Tensor, "Nf 3"] = f
    ...
    def remove_outlier(self, outlier_n_faces_threshold: Union[int, float]) -> Mesh:
        ...
        v_pos = torch.from_numpy(mesh.vertices).to(self.v)
        t_pos_idx = torch.from_numpy(mesh.faces).to(self.f)
    ...
    def _unwrap_uv(
        ...
        uvs = torch.from_numpy(uvs).to(self.v.device).float()
        indices = (
            torch.from_numpy(
    ...
    def set_vertex_color(self, v_rgb):
        assert v_rgb.shape[0] == self.v.shape[0]
        self._v_rgb = v_rgb
