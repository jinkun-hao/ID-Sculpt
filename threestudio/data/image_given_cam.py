...
class SingleImageDataBase:
    def setup(self, cfg, split):
        ...
        # camera_position = np.dot(mesh2std, cam_position.T).T
        # camera_position = torch.from_numpy(camera_position).unsqueeze(0)
        # shift = np.dot(mesh2std, shift.T).T
        ...
        # camera_position = np.dot(mesh2std, camera_position.T).T
        # camera_position = torch.from_numpy(camera_position).to(torch.float32)
    ...
    def load_images(self):
        ...
        mask_path = os.path.join(self.root_dir, 'mask.png')
...
@register("single-image-givencam-datamodule")
