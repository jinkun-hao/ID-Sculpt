...
class RealESRGAN:
    def __init__(self, device, scale=4, cache_dir=None):
    ...
    def load_weights(self):
        if self.cache_dir is None:
            model_path = hf_hub_download(repo_id=HF_MODELS[self.scale]['repo_id'], filename=HF_MODELS[self.scale]['filename'], cache_dir=self.cache_dir)
        else:
            model_path = os.path.join(self.cache_dir, HF_MODELS[self.scale]['filename'])
...
MODELS = {}
def sr(image: ndarray, scale: Literal[2, 4, 8] = 2, device=None, cache_dir=None):
    ...
    if scale not in MODELS:
        ...
        MODELS[scale] = RealESRGAN(device, scale=scale, cache_dir=cache_dir)
